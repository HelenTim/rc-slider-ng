...
export default function useDrag(
  ...
  allowCross: boolean,
): [number, number, number[], OnStartMove, number] {
  const [beginDraggingIndex, setBeginDraggingIndex] = React.useState(null);
  ...
  const updateCacheValue = (valueIndex: number, offsetPercent: number) => {
    ...
    if (valueIndex === -1) {
      ...
      const gouzValue = [-Infinity, ...next.values, Infinity];
      if (allowCross == false) {
        if (['ltr', 'btt'].includes(direction)) {
          if (offsetDist > 0 && gouzValue[valueIndex + 1] > gouzValue[valueIndex + 2]) {
            return;
          }
          if (offsetDist < 0 && gouzValue[valueIndex + 1] < gouzValue[valueIndex]) {
            return;
          }
        }
        if (['rtl', 'ttb'].includes(direction)) {
          if (offsetDist < 0 && gouzValue[valueIndex + 1] < gouzValue[valueIndex]) {
            return;
          }
          if (offsetDist > 0 && gouzValue[valueIndex + 1] > gouzValue[valueIndex + 2]) {
  ...
  const onStartMove: OnStartMove = (e, valueIndex) => {
    ...
    setBeginDraggingIndex(valueIndex);
  ...
  return [draggingIndex, draggingValue, returnValues, onStartMove, beginDraggingIndex];
}
